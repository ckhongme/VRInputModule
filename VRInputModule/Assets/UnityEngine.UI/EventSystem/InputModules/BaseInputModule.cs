...
namespace UnityEngine.EventSystems
{
    ...
    public abstract class BaseInputModule : UIBehaviour
    {
        ...
        // 用来处理当 pointer 的移出或进入某个 GameObject 时候，发送 enter 或 exit 事件
        protected void HandlePointerExitAndEnter(PointerEventData currentPointerData, GameObject newEnterTarget)
        {
            ...
            // if we have not changed hover target 判断是否事件是否是在同一个 target 上面
            ...
            if (currentPointerData.pointerEnter != null)
            {
                ...
                // 对从上次进入的对象到 commonRoot 之间所有的对象节点(不包括 commonRoot)依次发送 exit 事件；
            ...
            // 对从 newEnterTarget 到 commonRoot 之间所有的对象节点(不包括 commonRoot)依次发送 enter 事件
            currentPointerData.pointerEnter = newEnterTarget;
            if (newEnterTarget != null)
            {
