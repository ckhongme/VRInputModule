...
namespace UnityEngine.EventSystems
{
    public abstract class PointerInputModule : BaseInputModule
    {
        ...
        // 当前 Input Module 中的事件的生产者的 ID 以及事件数据对象 PointerEventData 之间的一个映射
        protected Dictionary<int, PointerEventData> m_PointerData = new Dictionary<int, PointerEventData>();

        // 根据某个事件生产者的 ID 获取 m_PointerData 映射的事件对象，返回 true 表示为该 ID 创建了一个新的事件，否则返回 false
        ...
        // 根据一个 Touch 生成一个点击事件 PointerEventData，同时指定当前帧是否有按下(pressed)、释放(released)操作。
        protected PointerEventData GetTouchPointerEventData(Touch input, out bool pressed, out bool released)
        {
            ...
            //如果创建新的 data 或者 手指刚刚触摸屏幕；
            pressed = created || (input.phase == TouchPhase.Began);
            //如果系统取消触控跟踪，原因如把设备放在脸上或同时超过5个触摸点，或者 手指离开屏幕
            released = (input.phase == TouchPhase.Canceled) || (input.phase == TouchPhase.Ended);

            //设置 PointerEventData 的相关信息
            if (created)
